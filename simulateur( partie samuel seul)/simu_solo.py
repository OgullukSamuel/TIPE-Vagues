...
class Environnement():
    def __init__(self,c_init=()):                      #condition initiales sous le format :([v0x,v0y],[a0x,a0y],pos0,angle0,t0,pts0)
        ...
        self.nb_sample=3                            #nb de points qu'on prend pour envoyer au rn ( pour prédire sur plusieurs checkpoints à l'avance)
    ...
    def get_state(self):
        ...
        if self.pts+self.nb_sample>=self.len_parcours:
            ...
            angles=np.concatenate(self.get_angle(v),np.zeros((self.nb_sample-self.len_parcours+self.pts)))
        else:
        ...
        return [np.dstack((dists,angles)).squeeze(),info]
    ...
    def render(self):
        """
        ici j'ai render l'influence de l'angle de la vague sur la trajectoire de mon bateau
        """
        def plotting(p,o,i):
            ax = plt.figure().add_subplot(projection='3d')
            ax.plot(p, o, i,label="aze")
        ...
        for j in range(10):
            ...
            time=[]
            for k in range(n):
                x,y = self.get_wave_force()
                a.append(np.linalg.norm(x))
                b.append(y)
                time.append(self.t)
                apo[k]=self.pos
                self.step([1,0.5])
            if j==0:plotting(apo[:,0],apo[:,1],time)
            else:self.ax.plot(apo[:,0],apo[:,1],time,label=f"{j*15}")
            self.angle_wave+=15
            
            self.k=np.array([np.cos(np.radians(self.angle_wave)),np.sin(np.radians(self.angle_wave))])
            self.A=self.amplitude_wave*self.omega_wave/(2*np.sinh(np.linalg.norm(self.k) * self.profondeur)) #L'amplitude" de la vitesse
            self.reset()
        self.ax.legend()
        plt.show()
