...
class Agent(tf.Module):  
    ...
    def fitting(self, states, actions,  adv , old_probs, discnt_rewards):
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            p = self.actornet(np.array(states), training=True)
            v =  self.criticnet(np.array(states),training=True)
    ...
    def training(self):
        for episode in range(self.episodes):
            ...
            for _ in range(self.epochs):
                self.fitting(states, actions, adv, probs, returns)  
