...
import os
import utilities
import parametres_DQL as parametres
...
class Agent(tf.Module):  
    ...
    def summon_net(self,nom):
        input1=tf.keras.layers.Input(shape=self.inputshape[1:],name="Input_state",batch_size=self.batch_size)
        out1=tf.keras.layers.Dense(32)(input1)
        out=tf.keras.layers.Dense(32)(out1)
        output=tf.keras.layers.Dense(2)(out)
        model = tf.keras.Model([input1], output,name=nom)
        model.compile(optimizer=self.opti, loss=self.lossing, metrics=self.metrique)
        model.summary()
        if nom=="Target": self.targetnet=model
        else:self.policynet=model
    ...
    def fitting(self):
        ...
        experiences = self.samplememory(self.batch_size)
        states, actions, rewards , next_states, dones =  utilities.extract_tensors(list(zip(*experiences))[1])
        #prédiction pour affiner les RN 
        target = self.policynet.predict([states],verbose=0)
        target_next = self.policynet.predict([next_states],verbose=0)
        target_val = self.targetnet.predict([next_states],verbose=0)
        #calcul des erreurs
        erreur = np.zeros(self.batch_size)
        ...
        #fit
        self.policynet.fit([states], target, batch_size=self.batchsize_fit,sample_weight=None,verbose=0, epochs=self.epochs)
    ...
    def addmemory(self, experience):
        ...
        target = self.policynet.predict_on_batch([np.expand_dims(state,axis=0)])[0]
        target_next = self.policynet([np.expand_dims(next_state,axis=0)])[0]
        target_val = self.targetnet.predict_on_batch([np.expand_dims(next_state,axis=0)])[0]
    ...
    def select_action(self,state):
        ...
        if self.episode >= self.supervised :                    #inutile ici, seulement présent si on compte faire une partie purement supervisée
            if self.get_explo_rate() > np.random.random():      #algorithme en exploration explo_rate % du temps, sinon en exploitation
                ...
                return tf.squeeze(tf.argmax(tf.squeeze(self.policynet(np.array([state]))))).numpy()  
    ...
    def training(self):
        ...
        for episode in range(self.episodes):
            ...
            while not done:
                ...
                next_state, rew, trunc, trunce, _= self.env.step(action)
                done=trunc or trunce
                rew=rew if not done else -rew*1000
                #reward.append(self.env.take_action(action))
                reward.append(rew)
                #next_state = self.env.get_state()
                self.addmemory(utilities.Experience(state,action,rew,next_state,done))
                state = next_state
                self.fitting()
            ...
            print(f"episode : {episode} || modele numero : {episode//self.checkpoint+1} || dernière récompense : {np.sum(reward)}")
